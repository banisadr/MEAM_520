%% haptic_damping_starter.m
%
% This script enables the user to feel virtual viscous damping using a
% PHANToM Premium 1.0 haptic interface.  The user's position is shown as a
% small red circle.  The user should feel a force proportional to and in
% the opposite direction as their velocity.
% 
...
% Set the viscosity of the virtual damping in newton seconds per millimeter.
b = 0.0005; % Keep this value positive and smaller than 0.002
...
set(gca,'Xtick',0:50:300,'ytick',-150:50:150,'ztick',0:50:300,'color',[.5 .8 1])
view(100,16);
box on
grid on
...
% Add a title.
title(['Haptic Damping by ' studentNames])
...
for i = 1:nCycles
    ...
    if i ~= 1
        dt = toc - lastGraphicsTime;
        dx = hx - hx_history(i-1);
        dy = hy - hy_history(i-1);
        dz = hz - hz_history(i-1);
        vx = dx/dt;
        vy = dy/dt;
        vz = dz/dt;
        vx_s = w*vx+(1-w)*vx_s;
