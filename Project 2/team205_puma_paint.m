...
studentnames = 'Bahram Banisadr, Wyatt Shapiro, and Val Cohen';
...
pumaStart('LEDMarkerStyle', '.', 'LEDMarkerSize', 10)
...
[~, xs, ys, zs, phis, thetas, psis, rs, gs, bs] = team205_get_poc(tstart);
...
allSolutions = team205_puma_ik(xs, ys, zs, phis, thetas, psis);
...
thetastart = team205_choose_solution(allSolutions, thetahome);
...
while(true)
    ...
    [~, x, y, z, phi, theta, psi, r, g, b] = team205_get_poc(thistory(i));
    ...
    % corresponds to one of the PUMA's six joints, in order.  Each column
    % is a separate solution to the IK problem.  Angles are in radians.
    allSolutions = team205_puma_ik(x, y, z, phi, theta, psi);
    
    % Choose the best solution based on the robot's current pose and other factors.
    thetas = team205_choose_solution(allSolutions, thetahistory(i-1,:)');
